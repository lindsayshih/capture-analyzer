	...
	public static void testStandardJDBC()
	{
		...
		try
		{
			...
			while (rs.next())  //still have records...
				out += rs.getString("tuple") + " " + rs.getLong("time") + "\n";
...
//			sqlExec.addParam(new Integer(8));
	...
	public static void testSimpleSelectWithParamsAndToString()
	{
		try
		{
			...
			sqlExec.addParam("tuple");
			SQLResults res = sqlExec.runQueryCloseCon("SELECT ? FROM all_flows");
	...
	public static void testDeleteAndInsert()
	{
		...
		sqlExec1.addParam(new Integer(123));
		sqlExec1.runQueryCloseCon("DELETE FROM all_flows WHERE time = ?");
		...
		String sql = "INSERT INTO all_flows (tuple, time) " +
		"VALUES ('a_b_c_d', '1234')";
