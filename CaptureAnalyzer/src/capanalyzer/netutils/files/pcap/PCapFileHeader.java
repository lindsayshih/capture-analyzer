...
public class PCapFileHeader
{
	...
	public void readHeader(InputStream in) throws IOException
	{
		...
		in.read(tmp);
		uint32MagicNum = ByteUtils.getByteNetOrderTo_uint32(tmp, 0);
		...
		uint32ThisTimeZone = ByteUtils.getByteNetOrderTo_uint32(tmp, 8);
		uint32Sigfigs = ByteUtils.getByteNetOrderTo_uint32(tmp, 12);
		uint32snaplen = ByteUtils.getByteNetOrderTo_uint32(tmp, 16);
		uint32LinkType = ByteUtils.getByteNetOrderTo_uint32(tmp, 20);
