...
public class ICMPPacket extends IPPacket
{
	...
	{
		super();
		setIPProtocol(IPPacketType.ICMP);
	}
	...
	public int getICMPType()
	{
		...
		{
			_isReadType = true;
			myType = ByteUtils.getByteNetOrderTo_uint8(myPacket,myIcmpOffset);
		}
		return myType;
	}
	...
	public int getICMPCode()
	{
		...
		{
			_isReadCode = true;
			myCode = ByteUtils.getByteNetOrderTo_uint8(myPacket,myIcmpOffset+ICMP_CODE_OFFSET);
		}
		return myCode;
	}
	...
	public int getICMPChksum()
	{
		if (_isReadICMPChksum == false )
		{
			...
			myCheckSum = ByteUtils.getByteNetOrderTo_uint16(myPacket,myIcmpOffset+ICMP_CHKSUM_OFFSET);
		}
		return myCheckSum;
	}
    ...
    {
    	if (myData == null && _isSniffedPkt)
    	{
    		myData = new byte[myPacket.length-(myIcmpOffset+ICMP_HER_LENGTH)];
    		System.arraycopy(myPacket,myIcmpOffset+ICMP_HER_LENGTH,myData,0,myData.length);
    	}
    	return myData;
   }
   ...
   {
	   return getIPHeaderLength()+getICMPLength();
   }
   ...
   public byte[] getICMPRawBytes()
   {
	   ...
	   if (myCheckSum == CALCULATE_CHKSUM || _isWriteChkSum == false)
	   {
