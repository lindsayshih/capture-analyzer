...
import capanalyzer.netutils.files.CaptureFileUtils;
...
public class FlowStruct
{
	...
	public FlowStruct(int theId,String theFileName,byte[][] rawData) throws NetUtilsException
	{
		myId = theId;
		myRawData = rawData;
		...
		if (rawData.length == 0)
			return;
		myFiveTouple = new FiveTuple(rawData[0], true);
		
	    myInitiatorIp = myFiveTouple.mySrcIp;
	    myDstIp = myFiveTouple.myDstIp;
    
		for (int i = 0; i < rawData.length; i++)
		{
			if (EthernetPacket.statIsIpPacket(rawData[i]))
			{
			...
			FiveTuple tmp = new FiveTuple(rawData[i], true);
			if (!tmp.equals(myFiveTouple) && !tmp.isOpposite(myFiveTouple))
				throw new NetUtilsException("Got more then one flow in cap");
		}
	}
	...
	private void loadData()
	{
		...
		try
		{
			myRawData = CaptureFileUtils.readCapRawData(myFileName);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	 ...
	 * @throws CapException
	 */
	public FlowStruct splitFlow(PacketSide side) throws NetUtilsException
	{
		loadData();
	...
	public String getDataStreamASKiwiLikeText(int maxPkt) 
	{
		...
		{
			toRetrun.append("FILES CONTAINS RETRANSMISSION, NOT FULLY SUPPORTED YET! \n");
    	}
		...
		for (int i = 0; i < maxPkt; i++)
		{
			...
			toRetrun.append("-----------------------------------------------------------------------------------\r\n");
			toRetrun.append(pktside.toArrow());
			if (pktside.equals(PacketSide.CLIENT_TO_SERVER))
				toRetrun.append(myFiveTouple.getMySrcIpAsString()+" : "+myFiveTouple.getSrcPort()+" , "+myFiveTouple.getMyDstIpAsString()+" : "+myFiveTouple.getDstPort()+"\r\n");
			else
				toRetrun.append(myFiveTouple.getMyDstIpAsString()+" : "+myFiveTouple.getDstPort()+" , "+myFiveTouple.getMySrcIpAsString()+" : "+myFiveTouple.getSrcPort()+"\r\n");
			toRetrun.append("-----------------------------------------------------------------------------------\r\n");
	...
	/**
	 * @return the flow packet sizes and directions as readable string
	 * @throws CapException
	 */
	...
	public String getOnlySizesAndDirections(int maxPkt) 
	{
		...
		toRetrun.append(myFiveTouple.toString());
		//ByteUtils byteutils = new ByteUtils();
		...
		for (int i = 0; i < maxPkt; i++)
		{
			...
			toRetrun.append("\n\n"+"packet number:"+i+"\n");
			toRetrun.append(getPktSideArrow(i+1)+"\n");
			toRetrun.append("data size:"+myData[i].length+"\n=================\n");
	...
	public String getOnlySizesAndDirectionsOnlyDataPkts(int maxPkt)
	{
		...
		for (int i = 0; i < maxPkt; i++)
		{
			...
			toRetrun.append("data average:"+avreage+"\n=================\n");
    ...
    public boolean isTransmissions(int maxpkts)
    {
        if (isUdp)
            return false;
        HashSet<Object> cleintseq = new HashSet<Object>();
